...
# separate per group
cor.test(df_FND$rrst_sensitivity, df_FND$sdq, method = "pearson") # r = -0.3828185, p-value = 0.01129
...
# separate per group
cor.test(df_FND$metascore, df_FND$sdq, method = "pearson")  # r = -0.36099, p-value = 0.0174
...
res # 
```
Seeing that the intake of psychotropic medication is both correlated to our outcome and predictor variables, we have to include this in our linear regression. For the others (sex, age, bmi) we do not have to include those, as they are not statistically correlated to both variables. For affective symptoms - there is a trend, hence we include as well.
...
sdq_lm <- lm(formula = sdq ~  rrst_sensitivity + psychotropic_medication,
                 data = df_FND) 
summary(sdq_lm)  # TREND for RRST sensitivity (p = 0.0571)

sdq_lm_affective <- lm(formula = sdq ~  rrst_sensitivity + psychotropic_medication + anx_dep_SUM,
                 data = df_FND) 
summary(sdq_lm_affective)  # TREND for RRST sensitivity (p = 0.0571) remains when including affective symptoms
...
#SDQ - metascore
sdq_lm <- lm(formula = sdq ~  metascore  + psychotropic_medication,
                 data = df_FND) 
summary(sdq_lm)  #  sig for metascore p = 0.047528 


sdq_lm_affective <- lm(formula = sdq ~  metascore + psychotropic_medication + anx_dep_SUM,
                 data = df_FND) 
summary(sdq_lm_affective)  # still sig for metascore after controlling for affective symptoms p = 0.047244
...
### 3.3.2) SDQ (full dataset with group as a factor)
...
sdq_lm <- lm(formula = sdq ~  rrst_sensitivity + group + psychotropic_medication,
                 data = df) 
summary(sdq_lm)  # SIG for RRST sensitivity (and group and medication)


sdq_lm_affective <- lm(formula = sdq ~  rrst_sensitivity + group + psychotropic_medication + anx_dep_SUM,
                 data = df) 
summary(sdq_lm_affective)  # RRST sensitivity ; secondary, also controlling for AFFECTIVE symptoms: rrst effect remains (along with group)!

# metascore
sdq_lm <- lm(formula = sdq ~  metascore + group + psychotropic_medication,
                 data = df) 
summary(sdq_lm)  # SIG association of metascore (along with group and medication)


sdq_lm_affective <- lm(formula = sdq ~  metascore + group + psychotropic_medication + anx_dep_SUM,
                 data = df) 
summary(sdq_lm_affective)  # still SIG for metascore; after controling for affective scores as well
```
### 3.3.3) SDQ (full dataset with group as a factor - in interaction with SDQ)
