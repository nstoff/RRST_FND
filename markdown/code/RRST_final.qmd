...
sdq_lm <- lm(formula = sdq ~  rrst_sensitivity + sex + anx_dep_SUM + psychotropic_medication + bmi,
                 data = df_FND) 
...
sdq_lm <- lm(formula = sdq ~  metascore + sex  + anx_dep_SUM + psychotropic_medication + bmi,
                 data = df_FND) 
...
# FND only
lm<-lm(rrst_sensitivity ~  sex + age + psychotropic_medication + anx_dep_SUM + bmi,
  data = df_FND)
summary(lm) # no sig
...
lm<-lm(metascore ~ group + sex + psychotropic_medication + anx_dep_SUM + bmi,
  data = df)
summary(lm)  #  no sig

# FND only
lm<-lm(metascore ~ age + sex + psychotropic_medication + anx_dep_SUM + bmi,
  data = df_FND)
summary(lm)  #  no sig
...
# predciting  traitPE uing RRST and MAIA with covariates
# FND only
lm<-lm(traitPE_iasRRST ~ age + sex + psychotropic_medication + anx_dep_SUM + bmi,
  data = df_FND)
summary(lm) #  no sig
