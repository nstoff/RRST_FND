...
cor1<-t.test(df$rrst_sensitivity[df$group=="1"], df$rrst_sensitivity[df$group=="0"])
cor2<-t.test(df$maia_total[df$group=="1"], df$maia_total[df$group=="0"])
cor3<-t.test(df$ias[df$group=="1"], df$ias[df$group=="0"])
...
results_interoceptiongroupdiff <- data.frame(
  Comparison = c("FND vs HC: rrst_sensitivity", "FND vs HC: MAIA", "FND vs HC: IAS"),
  Original_P = p_values,
...
## 3.4) SUPPLEMENTARY Regression - separate per group and controlling for ANX-DEP sum scores
...
# 4) Affective symptomology as RRST sensitivity confounders ?
