...
cor.test(df$breathlessness, df$rrst_sensitivity)
cor.test(df$asthma, df$rrst_sensitivity)
cor.test(df$unpleasantness, df$rrst_sensitivity)
cor.test(df$dizziness, df$rrst_sensitivity)
...
# 5) Modeling Confidence Data (R-script from Reviewer 3)
...
conf_fit <- glmmTMB(confidence ~ stimLevel * ResponseAccuracy * group + (1 | pcode),
                   data = rrst_data,
                   family = ordbeta(),
...
ggsave("5_Accuracy_interactions.tiff", 
       plot = combined_plot, 
       dpi = 600, 
       width = 6, 
       height = 4, 
