...
if vars.RRSTdevice == 1
    if (exist('RD','var')==0) || isnan(RD.respDevice)
        ...
        fprintf('\nCalibration of the RRST device... Please wait a bit...\n');
...
if vars.playNoiseSound
    soundFile = 4;          % Which sound to play? 1 Waterfall, 2 RainA, 3 RainB (duration 60min)
    switch soundFile
        case 1
        ...
        case 4
            thisSound = '03-White-Noise-60min.mp3';
    end
end
...
if strcmp(getenv('USERNAME'),'lcns')
    scr.screenID = 2; % use an external screen in full screen mode with the EEG laptop running the RRST
else
    scr.screenID = 0;
end
...
try
    ...
    if vars.RRSTdevice == 1
        fprintf('\nHere we move the device to a rand ITI pos...');
    ...
    while endOfExpt ~= 1                % General stop flag for the loop
        ...
        for thisAlt = 1:2
            ...
            if vars.wheresTheSignal(thisTrial) == thisAlt	% If this is the signal alternative
                ...
                if vars.RRSTdevice == 1
                    fprintf(['\nPosition sent:' num2str(thisTrialStim)]);
                ...
                % No load
                if vars.RRSTdevice == 1
                    fprintf('\nHere we bring the device back to no load...');
            ...
            while (GetSecs - ISIstart) <= vars.ISI
                ...
                if keys.KeyCode(keys.Escape)==1
                    % Reset resp device
                    if vars.RRSTdevice == 1
                        fprintf('\nHere it should be calibrating... (DO NOT INTERRUPT IT from CALIBRATING)');
                        calibrateResp(RD.respDevice);
                    end
                    % Save, mark the run
                    vars.RunSuccessfull = 0;
        ...
        if vars.abortFlag == 1
            % Reset resp device
            if vars.RRSTdevice == 1
                fprintf('\nHere we reset with Esc again... (calibrating)');
                calibrateResp(RD.respDevice)
            end
            % Save, mark the run
            vars.RunSuccessfull = 0;
        ...
        if vars.RRSTdevice == 1
            fprintf('Move the device to ITI pos (back)...');
            moveResp2ITIpos(RD.respDevice, unitscale);
        end
        ...
        if vars.ConfRating
            ...
            if vars.abortFlag       % Esc was pressed
                ...
                % Reset resp device
                if vars.RRSTdevice == 1
                    fprintf('Here we reset the device after abort... (calibrating)');
                    calibrateResp(RD.respDevice)
                end
                % Save, mark the run
                vars.RunSuccessfull = 0;
        ...
        while (GetSecs - StartITI) <= vars.ITI(thisTrial)
            ...
            if keys.KeyCode(keys.Escape)==1
                % Reset resp device
                if vars.RRSTdevice == 1
                    fprintf('Here we are resetting the device... (RECALIBRATING)');
                    calibrateResp(RD.respDevice)
                end
                % Save, mark the run
                vars.RunSuccessfull = 0;
        ...
        if ~rem((thisTrial-1), vars.PauseFreq) && ((thisTrial-1) ~= vars.NTrialsTotal)
            ...
            % Reset resp device
            if vars.RRSTdevice == 1
                fprintf('Here we calibrate following a break... (but it is not calib again)');
                %calibrateResp(RD.respDevice)
            end
            ...
            while (GetSecs - startPause) <= vars.pauseT
                ...
                if keys.KeyCode(keys.Escape)==1
                    % Reset resp device
                    if vars.RRSTdevice == 1
                        fprintf('Here we calibrate from an ESC press... (but again it is not calib)');
                        %calibrateResp(RD.respDevice)
                    end
                    % Save, mark the run
                    vars.RunSuccessfull = 0;
    ...
    % Reset resp device
    if vars.RRSTdevice == 1
        fprintf('Here we reset the device... (LET IT CALIBRATE BEFORE UNPLUGGING)\n\n');
        calibrateResp(RD.respDevice)
    end
