...
## Identifying the Outlier to exclude
...
sdq_lm <- lm(formula = sdq ~  rrst_sensitivity + sex + group + anx_dep_SUM + psychotropic_medication + bmi,
                 data = df) 
summary(sdq_lm)  # RRST sensitivity and GROUP and ANX-DEP; meaning these assocaition are not due to the difference of psychotropic medication
...
sdq_lm <- lm(formula = sdq ~  metascore + sex + group + anx_dep_SUM + psychotropic_medication + bmi,
                 data = df) 
summary(sdq_lm)  #  metascore and GROUP ; meaning ANX-DEP was maybe due to psychotropic mediation, but the other two assocaitions are not due to the difference of psychotropic medication
...
# FND only
lm<-lm(rrst_sensitivity ~  sex + age + psychotropic_medication + anx_dep_SUM,
  data = df_FND)
summary(lm) # no sig
...
# FND only
lm<-lm(metascore ~ age + sex + psychotropic_medication + anx_dep_SUM,
  data = df_FND)
summary(lm)  #  no sig
...
# FND only
lm<-lm(traitPE_maiaRRST ~ age + sex + psychotropic_medication + anx_dep_SUM,
  data = df_FND)
summary(lm) #  AGE
...
# FND only
lm<-lm(traitPE_iasRRST ~ age + sex + psychotropic_medication + anx_dep_SUM,
  data = df_FND)
summary(lm) #  no sig
